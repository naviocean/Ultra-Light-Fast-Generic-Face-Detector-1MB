...
int UltraFace::detect(unsigned char *raw_img, int img_w, int img_h, int img_c, std::vector<FaceInfo> &face_list, int num_class)
{
    classes = num_class;
    image_h = img_h;
    image_w = img_w;
    ...
    generateBBox(bbox_collection, out.output(0).get(), out.output(1).get());
...
void UltraFace::generateBBox(std::vector<FaceInfo> &bbox_collection, float *scores, float *boxes)
{
    for (int i = 0; i < num_anchors; i++)
    {
        ...
        {
            if (scores[i * (classes + 1) + j] > score_threshold)
            {
                FaceInfo rects;
                ...
                rects.score = clip(scores[i * (classes + 1) + j], 1);
                rects.label = j;
